...
public class GenericModel extends Model<GenericModel, GenericModelParameters, GenericModelOutput> 
    ...
    class GenericScoreContributionsTask extends MRTask<GenericScoreContributionsTask> {
        ...
        private void predict(Chunk[] cs, NewChunk[] ncs) throws PredictException {
            ...
            for (int i = 0; i < cs[0]._len; i++) {
                toRowData(cs, _fr._names, types, i, rowData);
    ...
    static void toRowData(Chunk[] cs, String[] names, byte[] types, int row, RowData rowData) {
        BufferedString str = new BufferedString();
        for (int col = 0; col < cs.length; col++) {
            final Object value;
            if (types[col] == Vec.T_STR) {
                value = cs[col].atStr(str, row).toString();
            } else if (types[col] == Vec.T_CAT) {
                if (cs[col].isNA(row)) {
                    value = Double.NaN;
                } else {
                    value = cs[col].vec().domain()[(int) cs[col].at8(row)];
                }
            } else {
